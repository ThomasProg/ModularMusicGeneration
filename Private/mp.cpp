...
midi_parse_track(struct midi_parser *parser)
{
  ...
  parser->in         += 8;
  parser->size       -= 8;
  parser->buffered_status = (midi_status)0;
...
midi_parse_channel_event(struct midi_parser *parser)
{
  ...
  if ((parser->in[0] & 0x80) == 0) {
    ...
    parser->buffered_status  = (midi_status) parser->midi.status;
...
midi_parse_event(struct midi_parser *parser)
{
  ...
  if (parser->in[0] < 0xf0) {  // Regular channel events:
    ...
    parser->buffered_status = (midi_status) 0;  // (cancels running status)

    if (parser->in[0] == 0xf0)
      return (midi_parser_status)midi_parse_sysex_event(parser);

    if (parser->in[0] == 0xff)
      return (midi_parser_status)midi_parse_meta_event(parser);
  }
  return MIDI_PARSER_ERROR;
}
